...
- `openai-adapter.ts` — Bidirectional conversion between OpenAI chat format and Anthropic Messages format. Handles OpenAI Responses API `input` field aliasing. Model and thinking are configured via env vars (`MODEL`, `THINKING_EFFORT`).
...
- **Single model routing**: A single model "Claude Code" is exposed to Cursor. The actual Claude model is configured via the `MODEL` env var (default: `claude-sonnet-4-6`). Thinking is always enabled, with effort controlled by `THINKING_EFFORT` env var (low=4096, medium=8192, high=16384 tokens).
- **Thinking tag filtering**: Claude may emit `<thinking>...</thinking>` tags in plain text even when thinking is not enabled via the API. These are stripped in both streaming (state machine in `stream-handler.ts`) and non-streaming (`anthropicToOpenai()` regex) paths.
...
See `.env.example`. Key vars: `PORT`, `ALLOWED_IPS` (comma-separated or `"disabled"`), `CLAUDE_CODE_EXTRA_INSTRUCTION`, `MODEL` (default `claude-sonnet-4-6`), `THINKING_EFFORT` (default `medium`; values: `low`/`medium`/`high`), `RATE_LIMIT_MAX_CACHE_SECONDS` (default 900 = 15 min), `RATE_LIMIT_SOFT_SECONDS` (default 300 = 5 min).
