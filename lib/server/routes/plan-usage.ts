...
async function estimatedWindow(
  sinceMs: number,
  windowMs: number,
  limit: number,
  now: number,
): Promise<PlanUsageWindow> {
  const { tokens, oldestTimestamp } = await getPlanWindowUsage(sinceMs);
 ...
 */
export async function handlePlanUsage(): Promise<Response> {
  ...
  const [fiveHour, weekly] = await Promise.all([
    estimatedWindow(now - FIVE_HOURS_MS, FIVE_HOURS_MS, quotas.fiveHourTokens, now),
    estimatedWindow(now - SEVEN_DAYS_MS, SEVEN_DAYS_MS, quotas.weeklyTokens, now),
  ]);
