...
describe("normalizeAnthropicRequest", () => {
  test("normalizes the public Anthropic model alias and rewrites invalid tool ids consistently", () => {
    ...
    const toolUse = normalized.messages[0]!.content as AnthropicRequest["messages"][number]["content"];
    const toolResult = normalized.messages[1]!.content as AnthropicRequest["messages"][number]["content"];
