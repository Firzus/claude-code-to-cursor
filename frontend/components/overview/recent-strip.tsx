      ...
      <CardContent className="px-6 md:px-8">
        <header className="mb-4 flex items-baseline justify-between">
          <div>
            ...
            <h3 className="font-display mt-2 text-2xl tracking-tight">Last 8 requests</h3>
          </div>
          <Link
            href="/usage"
        ...
        {isLoading && !data ? (
          ...
          <FadeStagger className="-mx-2 divide-y divide-border" stagger={0.04} y={4}>
            {data.requests.map((r) => (
              <div
                key={r.id}
                className="grid grid-cols-[1fr_auto] items-center gap-4 px-2 py-3 text-sm"
              >
